...
Use the subsampled data to get the code working. Once the code is working re-run it on the entire data set (which might take a while, but if your code works you should be able to just chill while it runs). 
...
The "optimal" parameter values can change depending on how you do cross-validation.


## Q4: Linear SVM

Use `method = "svmLinear2"` for the `caret` package to get linear SVM from the `e1071` package.
...
Use `method = "svmRadial"` for the `caret` package to get radial kernel SVM from the `kernlab` package (it's called `ksvm()` in the `kerlab` package). Type `?ksvm()` to see the documentation for this function.
...
Use cross-validation to select the optimal value of $C$ and $\sigma$ for SVM with a radial Kernel to find the "optimal" values of $C$ and $\sigma$. Report both the training error and the test set error for the model you chose in 5a.
