...
**1d**. How well does the cross-validation error approximate the true test set error? Describe in words how well the 
...
**2c**. Write a function that makes this tuning error curve plot with the three error curves (see below for the function skeleton). This question is asking you to take the code you wrote for the questions 1a, 1b and turn it into a function so you can look at how these plots change as the number of CV folds change.
...
Use `method = "svmRadial"` for the `caret` package to get radial kernel SVM from the `kernlab` package (it's called `ksvm()` in the `kerlab` package).


```{r}
gamma_values <- 10^seq(from=-5, to=5, by=1)
```
