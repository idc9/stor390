...
Use whatever functions you deem necessary to answer the question: Does `whales` meet the three criteria for tidy data given in the lecture? Type TRUE or FALSE in a code chunk.

```{r}
FALSE
```

It is a small dataset, so you can look at it directly or use `glimpse` to get the idea. 

It fails to be tidy for all three reasons: 

- Each row is data from one observer, not one event---which is our unit of observation here.

- Each cell contains multiple pieces of information.

- Each column is categorical information, not a variable in the sense of an attribute of each observation.
...
```{r}
whales <- gather(whales, key = species, value = info, -observer)
whales
```
...
```{r}
whales <- filter(whales, !is.na(info))
whales
```
...
```{r}
whales <- separate(whales, info, into = c('date', 'outcome', 'size', 'ocean'), sep = ',')
whales
```


## Q5: dplyr pop quiz
Do what you need to do to make the variables be of the following types (listed in order): integer, character, datetime (in correct order), character, integer, character.
...
```{r}
whales <- mutate(whales, 
                 date = parse_datetime(date, "%m/%d/%y"),
                 size = as.integer(size),
                 outcome = str_trim(outcome),
                 ocean = str_trim(ocean),
                 outcome = ifelse(outcome == "", NA, outcome)) %>% 
  mutate_each(funs(tolower), species, outcome, ocean)

whales
```


## Q6: dplyr pop quiz part 2
...
group_by(whales, species) %>%
  summarise(N = n(), avg_size = mean(size, na.rm = TRUE))
```
...
```{r}
unite(whales, col = id, observer, species) %>% count(id) %>% summary
```
...
```{r}
whales <- unite(whales, col = info, date, outcome, size, ocean, sep = ",") %>%
  spread(key = species, value = info)
