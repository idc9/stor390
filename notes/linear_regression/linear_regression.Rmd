...
```{r}
lin_reg <- lm(Sales ~ TV + Radio, advertising)

summary(lin_reg)
```

Simple linear regression puts a line in the data. When there is more than one x variable linear regression puts a [hyperplane](https://en.wikipedia.org/wiki/Hyperplane) through the data. So
