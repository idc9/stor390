...
Many of the examples in this lecture are from ISLR.
...
summary(lin_reg)
```
You'll notice R introduced a bunch of new variables (called [dummy varaibles](https://en.wikipedia.org/wiki/Dummy_variable_(statistics))) such as **mpaa_ratingPG**, **mpaa_ratingNC-17**, **best_pic_winyes**, etc. Notice, however, that there is no **best_pic_winno**. 

**Dummy variables**: replace a categorical variable, $x$, that has K categories with $K -1$ new **indicator variables $d_1, \dots, d_{K-1}$. For a given observation, $x$ the indicator $d_k$ is 1 if $x$ is in the k$th$ category. We only need $K - 1$ new variables since we can code the last category as all of the $K -1$ dummy variables are 0. 
...
**The upshot** is that once we have replaced categorical variables with dummy variables our data matrix is only numbers! You can see the new numerical matrix with the `model_matrix` function from the `modelr` package.


```{r}
library(modelr)
# here is a small example with just two covariates
# the -1 gets rid of the intercept term
model_matrix(data, imdb_rating ~ best_pic_win + critics_score - 1)
```
...
Now the resulting model is not linear in **imdb_num_votes**


```{r}
pred_df <- tibble(imdb_rating = unname(predict(lin_reg_trans)),
                  imdb_num_votes=data_trans$imdb_num_votes)

ggplot(pred_df) +
    geom_line(aes(x=imdb_num_votes, y=imdb_rating), color='red') +
    geom_point(aes(x=sqrt(imdb_num_votes), y=imdb_rating)) +
    xlim(0, 1000)
